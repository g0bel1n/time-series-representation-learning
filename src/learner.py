...
class Learner(GetAttr):
    ...
    def test(self, dl, weight_path=None, scores=None):
        ...
        if scores:
            s_vals = [
                score(cb.targets, cb.preds).to("cpu").numpy() for score in list(scores)
            ]
    ...
    def _prepare_data(self, test_data, Dataset=None, Dataloader=None, batch_size=None):
        ...
        if Dataset and Dataloader:
            ...
            if self.dls:
                ...
                test_dl = test_data  # assume test_data is already a form of dataloader
        return test_dl
    ...
    def fine_tune(self, n_epochs, base_lr=None, freeze_epochs=1, pct_start=0.3):
        ...
        assert (n_epochs > 0) | (
            freeze_epochs > 0
        ), "Either n_epochs or freeze_epochs has to be > 0"
        if not base_lr:
            base_lr = self.lr
    ...
    def freeze(self):
        ...
        if hasattr(get_model(self.model), "head"):
            ...
            for param in get_model(self.model).head.parameters():
    ...
    def to_distributed(
        ...
        **kwargs,
        ...
        self.add_callback(
            DistributedTrainer(
                local_rank=local_rank, world_size=world_size, sync_bn=sync_bn, **kwargs
            )
        )
...
def get_layer_output(inp, model, layers=None, unwrap=False):
    ...
    h_list = [
        getattr(model, layer).register_forward_hook(getActivation(layer))
        for layer in layers
    ]
